...
\begin{document}
	...
	\vspace{.5cm}
	\adjustbox{minipage=\linewidth}{
		\tiny{\VAR{''.join(LC_Settings.to_latex(bold_rows=True, column_format='rccc', na_rep='', index_names=False).rsplit('\n', maxsplit=2))}}
...
%	\end{figure}
	...
	\adjustbox{minipage=\linewidth}{
		\begin{tabular}{|p{1.6cm}|p{1.6cm}|p{1.6cm}|p{1.6cm}|p{1.6cm}|}
		 \hline
		 Created: \textbf{\VAR{Date.strftime('%d.%m.%Y').replace('.', ' ')}} & {\color{lightgray}Planned Start} & {\color{lightgray}Date Queued} & {\color{lightgray}Date Started} & {\color{lightgray}Date Done}\\
		 \hline
		\end{tabular}
	}
